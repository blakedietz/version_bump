...
| `src/version_bump/git.gleam` | Git access via `shellout` over the `git` binary. `parse_log` (pure) decodes a custom `--pretty` format delimited by ASCII unit/record separators; effectful helpers: `log_since`, `get_tags`, `current_branch`, `head_sha`, `list_branches`, `create_tag`, `push`, `get_remote_url`. |
...
| `hex.gleam` | Gleam-native plugin (no semantic-release equivalent) for publishing a Gleam package to **Hex**. `verify_conditions` (require `gleam.toml` with `description` + `licences`, and `HEXPM_API_KEY`, skipped on dry-run), `prepare` (rewrite the top-level `version` in `gleam.toml` via a pure `set_version`), `publish` (`gleam publish --yes`, reports the hex.pm URL). No `add_channel`: Hex has no dist-tags, so prereleases publish as ordinary semver prereleases. |
